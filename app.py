...
st.markdown("""
    ...
    .stTabs [data-baseweb="tab"] { height: 50px; white-space: pre-wrap; background-color: #11151c; border-radius: 4px 4px 0 0; gap: 1px; padding: 10px; }
...
def build_sfiral(r, hc, hs):
    # Генерация S-узла по вашему скрипту Blender
    res = 600
    r_a, pts = r / 2.0, []
...
def build_insect(wurf, current_states, l, w, r_base):
    # Соотношение сегментов ног через Золотой Вурф
    l_coxa = r_base * 0.3
    l_femur = l_coxa * wurf
    ...
    for i in range(6):
        ...
        # Амплитуда движения из нейросети
        ...
        traces.append(go.Scatter3d(x=[start[0], p1[0], p2[0]], y=[start[1], p1[1], p2[1]], z=[0, p1[2], p2[2]],
                                   mode='lines+markers', line=dict(color='#ffca28', width=8), name=f"Нога {i+1}"))
    return traces

# --- SIDEBAR: ГЕНЕТИЧЕСКИЙ КОД ---
with st.sidebar:
    st.header("🧬 ГЕНЕТИЧЕСКИЙ КОД")
    st.markdown("Это фундамент системы. Изменения здесь пронизывают все уровни: от вибрации нейрона до длины лапы робота.")
...
st.markdown("""
Путь реализации идеи: **Энергия (Сигнал) → Форма (Сфираль) → Жизнь (Кинематика).** Все уровни связаны коэффициентом Золотого вурфа ($W = 1.309$) и принципом антисимметрии.
""")
...
with tab1:
    ...
    with c1:
        ...
        ax1.plot(t_range, np.sum(all_states, axis=1), color='#ff4b4b', lw=2, label='Суммарный баланс')
...
with tab2:
    ...
    st.markdown("Проекция сигналов в структуру. Идеальное свитие без углов — признак чистого резонанса.")
...
with tab3:
    ...
    st.markdown("Робот управляется импульсами ФСИН. При резонансе движение максимально эффективно.")
