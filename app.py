...
# --- ГЛОБАЛЬНЫЙ СТИЛЬ ---
st.set_page_config(page_title="FSIN: Генетический код", layout="wide")
st.markdown("""
    <style>
...
class SfiralNetwork:
    def __init__(self, layer_size, base_weight=1.309, balanced=True):
        ...
        np.random.seed(42)
        # Веса, масштабированные Золотым Вурфом
    ...
    def get_states(self, t_val):
        ...
        if self.balanced:
            ...
            raw[half:] = -raw[:half] # Антисимметрия
        return raw

# --- ГЕОМЕТРИЯ (СФИРАЛЬ И РОБОТ) ---
...
def build_insect_legs(wurf, states, body_l, body_w, r_base):
    # Пропорции сегментов строго по Вурфу
    l_coxa = r_base * 0.3
    l_femur = l_coxa * wurf
    ...
    traces.append(go.Scatter3d(x=(body_l/2)*np.cos(t_b), y=(body_w/2)*np.sin(t_b), z=t_b*0, 
                               mode='lines', line=dict(color='silver', width=12), name="Корпус"))
    # Ноги (Радиальная симметрия)
    m_angs = [np.pi/4, np.pi/2, 3*np.pi/4, -np.pi/4, -np.pi/2, -3*np.pi/4]
    for i in range(6):
        m_a = m_angs[i]
        start = [(body_l/2)*np.cos(m_a), (body_w/2)*np.sin(m_a), 0]
        s = states[i % len(states)]
        ang = m_a + (s * 0.2)
        p1 = [start[0] + l_femur*np.cos(ang), start[1] + l_femur*np.sin(ang), 25*s]
        p2 = [p1[0] + l_tibia*np.cos(ang), p1[1] + l_tibia*np.sin(ang), -60]
...
with st.sidebar:
    ...
    st.markdown("Это фундамент. Изменения здесь пронизывают все уровни моделирования.")
    with st.expander("📡 Резонансные константы", expanded=True):
        mode = st.selectbox("Режим баланса", ["Антисимметричный (Резонанс)", "Случайный"])
        nodes = st.number_input("Узлы (Нейроны)", 6, 40, 10, step=2)
        wurf = st.number_input("Золотой вурф", 1.0, 2.0, 1.309, format="%.3f")
    with st.expander("🌀 Параметры Материи (Сфираль)", expanded=True):
        r_c, h_c, h_s = st.slider("Радиус", 10, 100, 50), st.slider("Высота витков", 10, 100, 30), st.slider("S-узел", 5, 50, 10)
    with st.expander("🕷️ Параметры Воплощения (Робот)", expanded=True):
        b_l, b_w = st.slider("Длина тела", 100, 300, 217), st.slider("Ширина тела", 50, 250, 166)
        t_phase = st.slider("Фаза движения", 0.0, 10.0, 0.0)

# --- ОСНОВНОЙ КОНТЕНТ ---
st.title("ФСИН: Сквозное моделирование")
st.markdown("**Энергия (Сигнал) → Форма (Сфираль) → Жизнь (Робот).** Все уровни связаны Вурфом $1.309$.")

net = SfiralNetwork(nodes, wurf, balanced=(mode == "Антисимметричный (Резонанс)"))
...
with tab1:
    ...
    with c1:
        ...
        ax1.plot(t_range, all_states, alpha=0.3); ax1.plot(t_range, np.sum(all_states, axis=1), color='#ff4b4b', lw=2)
    ...
    with c2:
        ...
        sns.heatmap(pd.DataFrame(all_states).corr(), cmap='RdBu_r', center=0, cbar=False, ax=ax_h); st.pyplot(fig_h)

with tab2:
    st.subheader("Геометрия Сфирального Узла")
    coords = build_sfiral(r_c, h_c, h_s)
    fig2 = go.Figure(data=[go.Scatter3d(x=coords[:,0], y=coords[:,1], z=coords[:,2], mode='lines', line=dict(color='#ffca28', width=6))])
    fig2.update_layout(scene=dict(aspectmode='data', bgcolor="#0e1117"), margin=dict(l=0,r=0,b=0,t=0)); st.plotly_chart(fig2, use_container_width=True)

with tab3:
    st.subheader("Биоморфный Гексапод")
    fig3 = go.Figure(data=build_insect_legs(wurf, current_states, b_l, b_w, r_c))
    fig3.update_layout(scene=dict(aspectmode='data', zaxis=dict(range=[-100, 100]), bgcolor="#0e1117"), margin=dict(l=0,r=0,b=0,t=0), height=700)
    st.plotly_chart(fig3, use_container_width=True)

st.download_button("💾 Экспорт Матрицы", pd.DataFrame(all_states).to_csv().encode('utf-8'), "fsin_genetic_matrix.csv")
